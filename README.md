...
📌 **Trang dự án**: [mira - klok](https://klokapp.ai/?referral_code=2AQB2MK6)
...
| `google.py`                    | Chương trình tự động đăng nhập google.   |
...
- Mỗi dòng chứa thông tin một profile theo cấu trúc:
  ```plaintext
  [tên_profile]|[email]|[mật_khẩu_ví_email]|[proxy (tuỳ chọn)]
  ```
  ...
  Trong đó:
    ...
    - `mật_khẩu_ví_email`: Mật khẩu đăng nhập tài khoản google
...
- Ví dụ:
  ```plaintext
  profile1|email1|pass1|38.154.227.167:2534             // Proxy (không yêu cầu mật khẩu)
  profile2|email2|pass2|user:pass@138.154.227.167:2534  // Proxy (yêu cầu mật khẩu)
  profile3|email3|pass3                                 // Không dùng proxy
  ```
...
- **Lần đầu:** Chạy **Set up (1)** để thiết lập cấu hình cần thiết, khi cần.
